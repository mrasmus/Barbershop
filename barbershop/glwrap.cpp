...
bool createglwindow(char *title, int w, int h, int bits, bool fs)
{
	...
	if(!glfwOpenWindow(w, h, 8, 8, 8, 8, 0, 0, fs?GLFW_FULLSCREEN:GLFW_WINDOW))
	{
	...
	glfwSetWindowTitle(title);	// Set the window title
	glViewport(0, 0, w, h);
	glMatrixMode(GL_PROJECTION);	// Change to the projection matrix,
	glLoadIdentity(); //reset the matrix and set up orthagonal projection (i.e. 2D)
	glOrtho(0, w, h, 0, -1, 1); // Paramters: left, right, bottom, top, near, far
